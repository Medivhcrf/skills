...
- **嵌套必须用解析器验，不能只数数量**：用 `html.parser` 之类解析整页，要求
  **0 处标签不匹配、栈深为 0**。只统计「`.sent` 数 == `.brk` 数」是不够的——
  多出一个 `</div>` 时计数照样对，但 `.para`/`.speech` 会被提前闭合，
  后面的拆解行掉出容器、编号底色与间距全部失效（症状：页面从某一句起突然「格式全坏」）。
- **对齐**：每句 `.ck` 数 == 该句 `.ly` 数。
- `pdfinfo "<pdf>" | grep Pages` —— 页数正常。**注意**：结构坏掉时页数会偏少，
  修好后页数可能变多，别把页数变化当成排版退步。
- `pdftotext "<pdf>" - | head -40` —— 中文不乱码、章节标题齐全。
- 手机版：抽查 390px 宽无横向溢出（可用 headless Chrome 截图，或确认 `scrollWidth == innerWidth`）。

> 用文字处理脚本改写页面时，以上第 1 条**每次都要跑**。踩过的两个坑：
> ① 用非贪婪 `(.*?)</span>` 取色块内容，遇到内嵌 `<span class="sup">①</span>`
> 会在内层提前收尾，取出未闭合的 span；
> ② 用非贪婪 `.*?</div></div>` 切 `.sent` 块，会漏掉关 `.sent` 的那个 `</div>`，
> **每句多一个**，逐句累积把祖先容器提前关掉。
> 两处都改为**按标签配平**扫描（见 `relayout.py` 的 `ck_contents` / `div_end`）。
