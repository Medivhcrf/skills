...
⚠️ 另一个历史坑（2026-09 修复）：个别页面里朗读 JS 的首行注释缺少结尾 `*/`，
把整段朗读代码都吞进注释里，导致脚本抛语法错误、逐句朗读**完全失效**
（点 🔊 只会播单词发音那套）。`add_read_aloud.py` 会把这类坏块一起截断并重插，
所以给老页面刷一遍就能修好；刷完用 `node --check` 验一下页内脚本块。

⚠️ 音频优先级：`read_aloud.js` 必须是「MP3 优先、系统语音兜底」。
系统语音优先会让所有朗读都变成 Windows 的机械男声（David），
仓库历史上来回改过两次（ee8c902 / c70968e），别再改回去。
"""
import io
import os
import re
