...
def main():
    ...
    textmap, all_text = {}, set()
    for f in files:
        h = io.open(f, encoding="utf-8").read()
        all_text |= targets(h)
    for t in all_text:
        textmap[t] = "words-audio/%s.mp3" % hashlib.md5(t.encode("utf-8")).hexdigest()

    missing = [t for t in all_text
               if a.force or not os.path.exists(os.path.join(adir, os.path.basename(textmap[t])))]
    print("文本 %d 条，需生成 %d 条，voice=%s" % (len(all_text), len(missing), a.voice))

    sem = asyncio.Semaphore(a.concurrency)
    tasks = [synth(t, a.voice, os.path.join(adir, os.path.basename(textmap[t])), sem) for t in missing]
    if tasks:
